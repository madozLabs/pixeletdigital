...
| Concurrence optimiste | Vérifiée en base (requête atomique) | Vérifiée en application seulement, écriture non protégée (Client/Quote/Invoice) | Réel bug | Critique | ✅ Corrigé pour Invoice cette nuit ; ✅ Quote/Client répliqué 2026-07-30 (`prisma-quote-repository.ts`/`prisma-client-repository.ts` : `updateMany` gardé par `version`, `ClientRepository`/`QuoteRepository.save()` retournent désormais `boolean`, `withMutableClient`/`updateQuoteStatus`/`convertQuoteToInvoice` renvoient `CONFLICT` si `false` ; `PrismaClientRepository.save` aussi passé d'`upsert` — sans garde possible sur la branche update — à create/updateMany explicites) |
...
- **B1 (complété 2026-07-30 — Quote/Client)** : même traitement appliqué à `PrismaQuoteRepository.save()` (`updateMany` gardé par version) et `PrismaClientRepository.save()` (converti d'`upsert()` — dont la branche update ne peut porter aucune condition au-delà de l'id — vers un `findUnique`+`create`/`updateMany` explicite, identique au patron Invoice/Quote). Les interfaces `QuoteRepository.save`/`ClientRepository.save` passent de `Promise<void>` à `Promise<boolean>` ; `updateQuoteStatus`, `convertQuoteToInvoice` et `withMutableClient` (partagé par tous les cas d'usage client) vérifient désormais ce retour et renvoient `CONFLICT` sur `false` au lieu de l'ignorer silencieusement. Tests de course ajoutés dans les deux suites d'intégration Prisma, sur le même modèle que celui déjà existant pour Invoice.
