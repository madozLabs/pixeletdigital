...
Status: Open owner decisions for Phase 1
Owner: Project owner
...
|---|---|---|---|---|---|---|---|
| ODR-001 | Approve official Pixel&Digital positioning and tagline | Homepage, services, SEO, claims and conversion copy | Final content approval | Use labelled draft structure; publish no final claim | Project owner | Open | Blocks stated gate |
| ODR-002 | Confirm Pixel&Digital launch service catalogue | Navigation, service pages, forms and evidence needs | Scope freeze and content approval | Include only owner-confirmed services | Project owner | Open | Blocks stated gate |
| ODR-003 | Confirm Kwaliti Print launch categories, materials, formats and capabilities | Discovery, quotation qualification and proof | Scope freeze and content approval | Treat all unconfirmed commercial facts as gaps | Project owner | Open | Blocks stated gate |
| ODR-004 | Decide pricing visibility | Copy, catalogue treatment, forms and qualification | Final UX/content and release | Quotation-only language; publish no price | Project owner | Open | Blocks stated gate |
...
| ODR-012 | Approve final logos and usage rules | Identity assets and cross-world orientation | Visual design and asset production | Preserve the three registered Pixel&Digital PNG files as review inputs only; do not infer missing variants, rules or rights | Project owner | Open | Partially progressed; still blocks stated gate pending authoritative vector/master files and formal usage rules |
| ODR-013 | Approve exact colours and type families | Brand system, contrast testing and production | Visual design and accessibility approval | Preserve directional red/white intent only; no exact values/fonts | Project owner | Open | Blocks stated gate |
...
| ODR-018 | Provide legal identity and required public notices | Footer, forms, ownership statements and compliance | Final content and release | Use no invented legal facts or final legal copy | Project owner | Open | Blocks stated gate |
| ODR-019 | Approve privacy terms, consent wording and data-retention rules | Forms, leads, media, analytics and operational handling | Final forms, analytics and release | Minimise requested data; keep final wording and retention unresolved | Project owner | Open | Blocks stated gate |
...
Three 1418 × 1418 RGBA Pixel&Digital PNG files and their visible presentations are now registered in `docs/01-brand/PIXEL_DIGITAL_LOGO_REGISTER.md`. The dominant opaque red observed in `P&D.png` samples as RGB 232, 27, 44 / `#E81B2C`; this is raster evidence, not an approved universal brand token. A dark-wordmark/light-background presentation is visible only as a reference, with no distinct fourth source file mounted. No vector/master files, font identification, clear-space, minimum-size, trademark, ownership or formal usage rules were supplied. ODR-012 therefore remains `Open`.
