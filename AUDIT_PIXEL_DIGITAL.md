...
**N4 — Présence collaborative temps réel** — ✅ déjà traité avant ce lot, sur plusieurs sessions antérieures, vérifié à nouveau ici le 30 juillet 2026 sans code à écrire :
- **Priorité 1 (détection de conflit, prioritaire selon le prompt lui-même)** : commit `c786999` ("fix(workspace): real optimistic concurrency where version already exists", 24 juillet) a corrigé les mutations qui faisaient un `version: {increment: 1}` aveugle (`Client.archive`, `EditorialItem.advanceWorkflow`, `Quote.updateStatus`, `Quote.convertToInvoice`, `Invoice.recordPayment`) pour vérifier `where: {id, version: expectedVersion}` — une écriture obsolète devient un no-op silencieux côté base (`P2025`/`count === 0`), détecté et transformé en message clair côté utilisateur. Le motif est maintenant répandu bien au-delà du minimum demandé par le prompt (juste « contenu » et « fiche projet ») : 15+ fichiers utilisent `expectedVersion` aujourd'hui, y compris `billing`, `leads`, `content` (pages/services/révisions), et **`projects/actions.ts:105-123`** (le cas explicitement cité par le prompt) — vérifié ligne à ligne : `updateMany({where: {id, version: expectedVersion}, ...})`, puis `if (updated.count === 0) return { status: "error", message: "Ce projet a été modifié par quelqu'un d'autre. Rechargez la page avant de réessayer." }` — exactement la contrainte du prompt (« pas de résolution automatique silencieuse »).
- **Priorité 2 (indicateur de présence)** : `src/app/workspace/_components/edit-presence.tsx` + `edit-presence-actions.ts`, ajoutés dans le commit `3d949ee` (26 juillet), complétés par la migration `20260728143000_add_edit_presence_context` (ajout de `contextLabel`, mergée dans le commit de tête `85fda89` de ce dépôt). Solution simple conforme à la contrainte du prompt (« pas de WebSocket sans justification ») : heartbeat client toutes les 30s (`heartbeatEditPresence`), fenêtre active de 75s côté serveur avec nettoyage des présences périmées dans la même transaction Prisma, table légère `edit_presences` (migration `20260726190000_edit_conflict_protection`). Badge « Aussi consulté par [nom] ([contexte]) » — le `contextLabel` optionnel permet même de préciser *quelle section* un collègue édite (ex. dans l'éditeur de contenu, écoute l'événement `cms:section-selected`), au-delà de la simple présence demandée par le prompt.
- **Scopé exactement comme demandé** : `PresenceEntityType = "PAGE" | "PROJECT"` — les deux entités explicitement nommées par le prompt comme « pages d'édition à plus fort risque de collision (édition de contenu, fiche projet) », pas plus. Composant réellement monté : `<EditPresence entityType="PAGE" .../>` dans `site-content/page.tsx:551` (éditeur visuel de contenu), `<EditPresence entityType="PROJECT" .../>` dans `projects/project-forms.tsx:132` (formulaire d'édition de projet, à côté du champ caché `expectedVersion`).
- Conforme à la contrainte « la détection de conflit fonctionne même si l'indicateur de présence est désactivé » : ce sont deux mécanismes indépendants (l'un lit/écrit `version` sur l'entité elle-même, l'autre une table `edit_presences` séparée) — aucune dépendance de l'un envers l'autre.
- Vérifié dans ce lot : `edit-presence.test.tsx` (comportemental : annonce les autres viewers, nettoie sa présence au démontage) fait partie des 589 tests unitaires verts ; `npm run test:integration:db` (17 fichiers / 85 tests, rejoué depuis zéro sur une base PGlite réelle) confirme que les deux migrations concernées (`edit_conflict_protection`, `add_edit_presence_context`) s'appliquent proprement. `tsc`/`eslint` propres.
- **Non fait dans ce lot** : rien à coder — le prompt d'origine est déjà entièrement couvert, sur son périmètre demandé (Page, Project). Non étendu (et volontairement laissé ainsi, comme documenté dès le commit `c786999`) : Task/Department/Team/JobPosition/TeamMembership n'ont pas de colonne `version` — hors du périmètre explicite du prompt (qui ne nomme que contenu et fiche projet), et Task en particulier est piloté par `position`/kanban plutôt que par un formulaire d'édition classique, un profil de risque différent. Vérification visuelle du badge de présence en session authentifiée non faite — bloquée par l'absence de PostgreSQL joignable sur cette machine (même limitation déjà documentée pour A2), compensée ici par la vérification d'intégration réelle sur base PGlite.
